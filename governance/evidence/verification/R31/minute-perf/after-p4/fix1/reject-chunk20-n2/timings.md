...
- 运行口径：`--chunk-days 20 --chunk-workers 1 --profile`，单因子超时 1500s（25min）；env：`FACTORLAB_DATA_BACKEND=ch FACTORLAB_MAX_MEMORY=8GB FACTORLAB_ST_DEGRADE=allow FACTORLAB_MINUTE_UNCOVERED=drop`；经 `governance/ops/heavy.sh` 闸。
...
|---|---|---|---|---|---|---|---|---|---|
| am_pm_vol | ERROR(rc=1) FACTORLAB_MAX_MEMORY；看门狗仍会按 chunk 边界协作检查。 | —（未完成） | — | — | — | — | — | — | 344MB |
