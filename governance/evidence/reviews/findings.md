...
| R37-AGG-I1 | I | **门控型成员 z 爆尾 → 脉冲式主导聚合尾部**：`momentum_20d_turnrank_top2`（换手前 2% 门控 × 20 日动量；98% 行=0）经截面 z 标准化后，激活票 z 达 ±11（常数质量 +0.0969）；样本 2026-07-31 600105.SH z=-11.28×w(-1/9) 贡献 +1.2537＝复合分 1.6379 的 **76.5%**（其余 8 员合计 0.384；无该贡献排名 ~800 名落选，有此贡献当日第 1）。全期平均 |z| 最小（0.206）但尾部主导 select TopK——等权线性聚合对零膨胀/门控成员无稳健性处理。建议：composite 成员级可选 clip/cs_rank 变换（spec/params），或门控型成员 process 改 cs_rank；口径文档明示成员分布假设【实测】 | `research/composites/implementations/linear_rank.py`（linear）；成员 spec（产物区 `experiments/r37_5y/ref/momentum_20d_turnrank_top2_5y.yaml`）；证据 `governance/evidence/verification/R37/acceptance/ref10/member-tail-check.md` | open |  |  |
