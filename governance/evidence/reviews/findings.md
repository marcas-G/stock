...
> 以下为 R31 上表化条目（2026-09-20 补录：原 bullet 格式同步器不可见，GitHub issue 化）。
...
| R37-EXEC-I3 | I | **长窗口读路径 schema 推断崩溃**：`load_adj_detail_window` 用 `pl.DataFrame(rows, schema=[列名], orient="row")`——只给列名时按前 100 行推断 dtype；5 年窗口前段全 NULL 的列推断成 Null，后续出现小数（2.564）即 `ComputeError`，5 年回测被整单打断。修复：显式 dtype schema（不再随行序/窗口长度漂移）【实测】 | `platform/src/factorlab/adapters/read/market_open.py`（load_adj_detail_window）；修复 commit `263b24c`；测试 `platform/tests/test_backtest_ca_gate.py::test_detail_loader_robust_when_prefix_all_null`；复跑证据 `governance/evidence/verification/R37/acceptance/5y/README.md` | verified | `263b24c`：显式 dtype schema；`cd platform && .venv/bin/python -m pytest tests/test_backtest_ca_gate.py -q` → **34 passed**；5 年回测复跑 `ok:true`（255 events / +41.66%，见证据 json） | verified（2026-09-20 复跑：先红后绿测试 + 5 年回测成功） |
| R37-REF-I1 | I | **参考库方向元数据与边际 IC 符号不一致（9/37）**：`summary.direction` 与 5 年面板 `corr(signal, forward_return_1d)` 符号相反者 9 例（rsi_reversal_14、reversal_20d_overnight、intraday_auction_range、abs_auction_premium、cross_count、cn_spread、turnover_cv、volume_autocorr、volume_entropy）。判断：部分 direction 系**条件检验**（resIC/条件 t，见 R31-STAT-I1 符号翻转现象）而非边际口径产物；线性聚合（X@w）按元数据取符号会**减掉 alpha**。影响：本次聚合演示按边际 IC 符号定向（口径已记 composite spec 注释）。请求：参考库按"方向口径"字段化（marginal/conditional）或线性使用另注 sidecar；档案同步【实测】 | `governance/evidence/verification/R37/acceptance/README.md`；spec 注释 `research/composites/specs/ref_daily_9_5y.yaml`（产物区）；聚合证据 `governance/evidence/verification/R37/acceptance/ref10/` | open |  |  |
| R37-AGG-I1 | I | **门控型成员 z 爆尾 → 脉冲式主导聚合尾部**：`momentum_20d_turnrank_top2`（换手前 2% 门控 × 20 日动量；98% 行=0）经截面 z 标准化后，激活票 z 达 ±11（常数质量 +0.0969）；样本 2026-07-31 600105.SH z=-11.28×w(-1/9) 贡献 +1.2537＝复合分 1.6379 的 **76.5%**（其余 8 员合计 0.384；无该贡献排名约 366 名落选，有此贡献当日第 1）。全期平均绝对分最小（0.206）但尾部主导 select TopK——等权线性聚合对零膨胀/门控成员无稳健性处理。建议：composite 成员级可选 clip/cs_rank 变换（spec/params），或门控型成员 process 改 cs_rank；口径文档明示成员分布假设【实测】 | `research/composites/implementations/linear_rank.py`（linear）；成员 spec 与证据 `governance/evidence/verification/R37/acceptance/ref10/member-tail-check.md`（含公式/分布/样本） | open |  |  |

## R37 参考库治理发现（2026-09-21，reference library audit）

> 背景：42 员全量 5 年聚合回测期间，发现参考库入库判据只有"独立性（corr）+ 条件残差显著性（resic）"，
> 缺逐员裸指标底座（raw IC/t/IR、方向一致率、覆盖、持有期口径）。审计与过滤对照证据：
> `governance/evidence/verification/R37/acceptance/ref10/reference-audit-20260921.md`。

| ID | 级 | 问题 | 关键位置 | 状态 | 修复说明（团队填） | 复查 |
|---|---|---|---|---|---|---|
| R37-REF-I2 | I | **参考库入库判据缺"裸指标过滤/底座"**：42 员审计中 10 员裸指标未过——3 员为 spec 方向标错（volume_autocorr/amihud_autocorr/overnight，已修 spec 并验证一致率 63%/67%/74%），5 员过滤剔除（abs_auction_premium、turnover_cv、parkinson_ratio、volume_entropy、time_of_max_r），另 2 员条件/持有期口径冲突（tail_amt_share、time_range_conc，按 20d 符号校验保留）。过滤对照：37 员 vs 42 员 OOS +55.46% vs +52.85%、Sharpe 0.84 vs 0.81、MaxDD -16.79% vs -17.74%【实测】 | 审计证据 `governance/evidence/verification/R37/acceptance/ref10/reference-audit-20260921.md`、`governance/evidence/verification/R37/acceptance/README.md`；方向修正记录见各 spec 注释 | open |  |  |

## R43 组合评估发现（2026-09-25，QA）

| ID | 级 | 问题 | 关键位置 | 状态 | 修复说明（团队填） | 复查 |
|---|---|---|---|---|---|---|
| R43-PORT-I1 | I | **porteval 零方差指标输出非标准 JSON**：平收益场景使 `simulate` 返回 `ir=NaN`、`sharpe=Infinity`；生产 writer 默认 `json.dumps` 将其写成非标准 `NaN`/`Infinity`，严格 JSON 解析器拒绝 portfolio 产物【实测】 | `research/tools/porteval/pv_engine.py:188-190,231`；`research/tools/porteval/run.py:110`；复现 `governance/evidence/verification/R43/tester/repro_porteval_nonfinite_json.py`，输出 `governance/evidence/verification/R43/tester/repro_porteval_nonfinite_json.txt` | open |  |  |
