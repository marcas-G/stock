...
| 9 | **Plan DQ 数据质量** | M1（daily 纵切）→ M1.5（18 日/adj/1.25M）→ M1.5c（delta 门控） | ✅ + 🔶 | `2026-09-18-data-quality-pipeline*`；R32/R33 | **M2**（minutes 三角验证+校准）、**M3**（BJ 1,115/前1996 2,658 历史修复） |
...
3. 台账现状：**114 行 · verified 112 · open 2 · fixed-claimed 0 · 复查列覆盖 114/114**；`make gates` exit 0
...
2. **Plan DQ-M3**：BJ 1,115 行用 bars_1m 重建 amount/volume；前 1996 残余 2,658 逐码处置；R32/R33 审计按 daily-v2 重跑
...
6. **提交本次门豁免改动**（`check_dataiface.py` + 4 处 CX 注释措辞；当前在工作区未提交）——否则干净检出仍会红
...
- **未提交改动**（团队/我）：检查脚本与暂停豁免（`check_dataiface.py`/`check_reviews.py`）、CX 注释措辞 4 处、`findings.md` R35 回填、R35 证据、本 STATUS 与 pending #23。
...
| PLAN-DQ-M3 | [#8](https://github.com/marcas-G/stock/issues/8) | 历史残余定向修复 |
