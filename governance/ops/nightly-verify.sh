...
#
# 流程：经 `governance/ops/heavy.sh`（内存闸 + nice）执行
#   `bash governance/ops/verify.sh --profile <NIGHTLY_PROFILE>`
...
#   NIGHTLY_VERIFY_CMD   覆盖被执行的命令（测试/演练用；默认 heavy.sh + verify.sh）
#
...
if [ -n "${NIGHTLY_VERIFY_CMD:-}" ]; then
    ...
    VERIFY_CMD=("$OPS_DIR/heavy.sh" bash "$ROOT/governance/ops/verify.sh" --profile "$PROFILE")
fi
