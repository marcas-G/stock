...
A3. ✅ **2026-09-17 关闭（fix(tools) + docs/verification）**：`convert_minutes_to_parquet`
   新增 `_source_relation`/`_commit_status`——已提交月按回执 manifest（`_daily_manifest.parquet`
   逐日源 zip name+size）与当前源比对：新增/同名替换 → 清理重转吸收；源归档缺失 →
   `SourceRollbackError` fail loud（不自动重转，避免抹掉已提交历史日）。真实重转 202609
   （13 日，含 9/17）+ 重灌 CH：bars_1m max=2026-09-17、202609=17,344,800 行；
   check-day 2024-01-15 max|Δ|=0；9/17 CH×本地全列逐值=0；`make reconcile` 全库一致。
   证据 `governance/evidence/verification/R30/minute-month-absorb/`。**连带缺口见 A4**
   （重转后 `ingest_bars` 仍按旧断点跳过，本次交付为手工重灌）。原登记保留如下——
...
A4. ✅ **2026-09-18 关闭（fix(tools) + docs/verification）**：`ch_ingest` bars_1m 月断点
   ...
   `make data-update` verify 阶段红（实测：手工重转 202609 后若不删断点，ingest
   报「已完成 80，待跑 0」；本次交付已删 `bars_1m_202609` 键手工重灌）。
   未决因：给 bars/tick 任务断点加源指纹（parquet name+size+mtime 摘要，stat 级
...
## tick 事件因子挖掘停用（2026-09-18，用户裁决）

- **背景**：minute 库饱和后转 tick/L2 方向，做了**事件原生日聚合**
  （`platform/tools/lob_fact/pipeline/event_daily.py`，TDD 7 测试绿，含真实数据独立交叉验证；
  时段化 am/pm/close30 版已实现，批算跑到 242/250 日）。
- **实证结论（停用的依据）**：300 股 × 250 日样本上，18 个事件统计（撤单洪峰/挂单存活/
  扫单强度/flash cancel 等）全期 raw IC 最大 |t|=2.76（cancel_match_ratio，18 重测试下约 10%
  概率为噪声）；对 minute 库 17 员残差化后 resIC 全部 |t|≤1.8——**事件计数信息与分钟库高度
  重叠，无显著增量**；tick 仅一年历史，检验功效天然不足。
- **状态**：用户令"先不看了也不挖了"→ 批算已停（`pkill event_daily`），半成品
...
24. **BJ（北交所）与 1996 年前数据出研究范围（2026-09-20 用户拍板）**
    口径：研究/发布范围 = `trade_date >= 1996-01-01` 且 `~code.endswith('.BJ')`；
    实计 **17,787,921 行 = 全表 97.57%**（出范围：BJ 270,183 行 + 前 1996 172,128 行，
    二者无重叠；已用 `daily_fact` 全表实算）。
    影响：**DQ-M3 残余从 3,828 行缩至 55 行**（1996+ 非 BJ；BJ 1,115 与前 1996 2,658
    不再要求修复）；health 发布链按新范围发布；research 默认起始 1996-01-01、universe 不含 BJ。
    数据处置：**保留在盘、不删除**，仅不纳入范围（将来要用可回补）。
