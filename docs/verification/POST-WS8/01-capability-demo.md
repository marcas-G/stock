...
## 10. 追加（同日）：门自身的两类缺陷（commit e83fb2d）

按用户真实用法"写因子 → 跑结果"实测，暴露的不是功能而是**门**的问题：

| # | 缺陷 | 危害 | 修法 |
|---|---|---|---|
| 5 | `lint` 拿**未替换**文本过 AST 门 | 文档化 `params`/`${name}` 模板假报"语法错误"——全库 152 spec 中 **15 个**失败（全是 vol_run_energy_*），写因子第一条命令即误报（真错假错无法区分） | 与引擎同序（`substitute_params` → `validate_formula`）+ 校验范围补全池公式与宏体；复验 **152/152 通过** |
| 6 | Web 集成测试 fixture 只判 `results/` 目录存在 | 用户首跑自己的因子 → 3 个用例由 skip 变 **fail**（断言历史档 m4b_smoke 等，实际是用户自己的产物） | 按**具体产物**判 skip（`missing_fixture_factors`）+ 守卫用例 |

**新增缺陷类归纳**：**门本身也会假失败**——"真错与假错无法区分"和"用户正常操作触发假红"
都是交付级缺陷。门必须与实现**同序**、按**具体前提**判定，而不是按近似条件。

- 终态门：平台全量 **2486 passed / 13 skipped**（跳过数与基线一致；+24 用例逐项登记）。
- 写因子→跑结果闭环实跑：新因子 `turnover_tail_shape`（含自定义 `def` 处理函数）
  n_weeks=45 / ic=0.0110 / spread=+0.00089；演示产物已移出 `results/`（保持工作树初始态）。
