...
从逐笔消息流**逐单事件级重建 A 股订单簿**（lob_fact）：价格档级状态流 + 耗尽元数据 + 分钟检查点，服务盘口因子。范围决策（2026-09-09 用户拍板）：**SZ 撤单走直接通道 + 修复上游**（W0 增补 tick_fact/cancels 表），SZ 与 SH 同构逐单事件级重建；快照 10 档作为锚点验证/分类。内存硬约束（用户指定，共享机；**2026-09-11 用户决策翻倍**）：批算总驻留 ≤64GB、常态 ≤48GB（原 24/32GB —— 预算翻倍后批算 worker 2→4）。
...
**W5 修订（2026-09-11，用户决策内存预算翻倍 24/32 → 48/64GB）**：worker 数 2→4（40 核机、负载 ~11；4 × 实测平均 8.5GB ≈ 34GB 常态、投影峰 ~51GB ≤64GB 硬门）；**派发低水位 `LOW_WATER_KB` 8→16GB**（4 worker 同刻在飞时单 date 切片峰 ~12.8GB，旧水位下"在飞 × 新增"会过冲 OOM 防护）；审计门 `NORMAL_LIMIT_KB`/`HARD_LIMIT_KB` 同步翻倍，冻结测试 `test_run_lob_batch.py::test_resource_gates_frozen_for_doubled_budget` + `test_audit_w5.py`（改小门 = 静默放松验收 → 必败）。前段 2-worker 实测峰 23.9GB（和）/ 24.1GB（全和）落在原门内（备忘 §5 两段实测表）。采样 30s / STALL 2400s / RG 几何不变。
...
- **字节级重跑比对 PASS**：`--force` 二跑（run 20260910_082834_33436）vs 首跑
  ...
  → 确定性非偶然。
- **体积（W2 96B/行模型被实测取代；~~预算 ≤1.5×源 达标~~ → 2026-09-11 更正：超预算）**：
  月总 8,238.5MiB（events 7,104.1 + sweeps 924.6 + ckpts 209.8）vs 源 5,355.6MiB =
  **1.5383×**。**更正（2026-09-11）**：原记 "1.461× 达标" 有两处错 —— ① 单位实为
  MiB（`du`/字节数 ÷2²⁰）却记成 MB；② 比值算错（1.461 = 按 2¹⁰ 混算）。8,238.5 /
  5,355.6 = 1.5383 → **超 1.5× 预算，W4 "体积达标" 结论作废**；磁盘内容已核对
  （3 表实际字节 vs 两次 run summary 记录一致，0 字节/0 行差）= 仅记账错，非数据问题。
  收口路径 = W5 zstd9（§3.2/§3.4），预计 202608 → ~1.4657×。关键杠杆 = 行组几何：
  逐 code 组 12.1B/行 → 1,048,576 行组 10.1B/行（−12.7%，zstd 上下文）；
...
- **RSS 审计（30s×~970 采样）**：单 worker 峰 12.4GB / 双 worker 同刻和峰 22.7GB
  ≤24GB normal ✓（≤32GB hard 余量；**旧门**）。W2 '1.5-2.5GB/worker' 估算被推翻 →
  worker 数冻结 2（§3.4 已修订；**W5 2026-09-11 用户决策预算翻倍 24/32→48/64GB →
  4 worker，见 §3.4 "W5 修订" 段**）。
