...
- 池公式在**完整上市骨架**上求值（不套 universe mask）：
  ...
  - GP 算子：**第一版开放**（2026-09-06 复核点 3 定案）。分组键 = catalog 注册的
    per-code 属性 `industry`（见 §5.4）——行业属性独立于池条件，组内截面在完整骨架的
    同行业内求值，无自指；"行业内相对强弱"（如 `gp_rank(ts_delta(close,5)) > 0.9`）由此
    可表达且语义确定。行业为空的股票不进任何组：组内统计不含它，依赖组统计的池条件
    对它不成立（需 `industry != null` 类条件显式处理）。引用 gp_ 而数据面无分组键时
    编译报错并指认缺哪个键。
...
- `UniverseSpec` 增加 `formula: str | None` 分支。定稿决策：**互斥**（2026-09-06
  复核点 1 定案：`_exactly_one_universe` 扩为 ref/codes/rules/formula 四选一），
  池只有一个来源，避免两套语义叠加的文档面。
  需要静态硬过滤（如 exclude_st 当骨架约束）的场景：视为骨架层数据问题，
  骨架层维持既有静态机制，不由 spec 混叠。
  需要"名单 ∧ 条件"（如"只在沪深300成分里且站上均线"）的场景：名单经注册制以
  **成分标志字段**（per-code 0/1 属性，与 §5.4 的 industry 同一机制）进入公式——
  不设第二个 spec 来源。
...
### 5.4 第一个 per-code 属性注册：industry（第一版就做，2026-09-06 复核点 3 定案）

- **来源**：本库 `stock_basic.industry`（静态属性列，rebuild STOCK_BASIC_FIELDS 之一）。
- **近似声明（诚实标注）**：industry 是**当前值，非严格 PIT**——行业重分类低频但存在，
  严格做法需行业历史快照表并登记各期生效时点。第一版以当前值近似并写入目录语义，
  升级项 = 行业历史表注册后替换数据源，池/主公式语义随替换而严格化（与 pit_qfq 同理念，
  不阻塞第一版）。
- **接入**：load 层把 industry 作为 per-code 属性 join 进行情面板——**仅在公式引用它时
  才拉取**（防列膨胀），duckdb|ch 各一个编译函数；主公式与池公式共用同一数据面。
- **效果**：池公式与主公式的 `gp_` 算子族由此真正可用（现状：算子已注册但数据面
  无分组键，公式实际不可用）——"行业中性因子""行业内相对强弱池"第一版即可表达。
...
| G2 公式化池 | `_ALLOWED_RULES` 3 静态键 | `universe.formula` 布尔门（ref/codes/rules/formula 互斥四选一）+ 骨架求值 + 动态 mask 来源；池公式开放 elementwise/ts_/cs_/gp_ | spec.py、data/universe.py、compute.py:374-380 |
...
| G5 语义回归网 | 静态池测试 | 动态池语义锁（4.4 不变式、池公式未来门、池内 CS 与全骨架 CS 区分、gp_ 组内语义：组缺失/组内 null/industry 当前值近似） | 新测试组 |
| G6 行业数据面 | industry 在 stock_basic 静态表，公式不可引用 | per-code 属性注册 + 按需 join 进面板（duckdb\|ch 编译函数对）+ 目录登记（当前值近似声明） | data/source.py 读路径、catalog、§5.4 |
...
- 行业属性的严格 PIT 历史：第一版以 stock_basic 当前值为近似（§5.4），
  行业历史表经注册后替换升级——近似范围与影响写入目录语义与测试。
- tick/1m 级因子与池条件：数据管道未接，属"新数据注册"流程范畴（5.3），不在本定稿内实现。

## 9. 复核点定案记录（2026-09-06，全部定案，实施不再回头问）

| # | 问题 | 定案 |
|---|------|------|
| 1 | universe.formula 与静态 codes/ref/rules 可否并存 | **互斥**（四选一）。"名单 ∧ 条件"经注册制用成分标志字段进公式（§4.3） |
| 2 | 多输出名字空间 | **全局唯一**，不允许两个输出重名（报错指认冲突） |
| 3 | 第一版池公式要不要分组截面 | **要**——行业内相对强弱第一版即可写（§4.2/§5.4，industry 数据面进第一版范围） |

**已知近似（不阻塞，写入目录与测试，升级项单列）**：
- industry = stock_basic 当前值，非严格 PIT（行业重分类低频；严格化需行业历史快照表，见 §5.4）。
