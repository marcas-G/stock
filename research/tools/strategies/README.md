...
| `l5_rules.py` | **L5 规则层 V1**：`max_hold` 调仓日粒度近似（边界见 `$QUANTRESEARCH_ROOT/dossiers/strategies/_l5-rules.md`） | 目标组合历史 + 交易日历 |
...
**策略 spec/档案**（R37 起在产物区 `$QUANTRESEARCH_ROOT`）：`strategy/*.yaml` +
`dossiers/strategies/*.md`（索引 `index/strategies.md`，`make index` / `make index-check` /
G-INDEX 有 --check 门）。
...
**文档**：`$QUANTRESEARCH_ROOT/dossiers/strategies/`（含策略口径与结论）。
