...
| `l5_rules.py` | **L5 规则层 V1**：`max_hold` 调仓日粒度近似（边界见 `research/docs/strategies/_l5-rules.md`） | 目标组合历史 + 交易日历 |
...
**策略 spec/档案**：`research/strategy/*.yaml` + `research/docs/strategies/*.md`（索引
`docs/index/strategies.md`，`make index` / G-INDEX 有 --check 门）。
