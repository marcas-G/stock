...
2. `factorlab run $QUANTRESEARCH_ROOT/factor/<族>/<stem>.yaml`（或 `--no-backtest` 快速版）
...
```
factorlab run $QUANTRESEARCH_ROOT/factor/<族>/<stem>.yaml   # 计算 + 评估 + 分层回测
...
**因子档案（md）**：每个因子在 `$QUANTRESEARCH_ROOT/dossiers/factors/<族>/<stem>.md` 维护一份研究档案
（模板：`$QUANTRESEARCH_ROOT/dossiers/factors/_template.md`），与 `$QUANTRESEARCH_ROOT/factor/<族>/<stem>.yaml` 同族同短名并行存放：
