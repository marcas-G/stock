...
> 版本数字自 `runs/platform/composites/linear_rank/{provenance,summary}.json`。
...
| 版本项 | 值 |
|--------|----|
| definition_hash | `1ef5402b450c5b8c5ca59ae1c5cc018689693a1897deefa823b20725179368ad` |
| cache_key（H = Spec+Impl+Params+MemberHashes） | `ccc867e0bc5bb579cf179f460b977aec646e6d6406c48525f2f231fbf5456307` |
...
| output_hash | `b24a2e960fe64d143c0575bdd0e14a260bdc4de6e828222d9bfcaa829e1667a4` |
| environment.lock_hash | `6b00f85b732425c8376cde68bc48b415a0d0f0ff3b48ac16572dbd5533df6257`（C2 已接线） |

## 2. 方法与参数

- **方法**：平均秩等权合成——每列 `rank("average")/N`（polars）后行均值（`y = mean_k rank_k/N`）。
  **口径注**：C1 runner 单次把整面板 X 交给 compute（X-only 契约，compute 看不到日期分组），
  故此处为**列内全表** rank 归一，不是逐日截面 rank；逐日截面口径的等权 rank baseline
  由评估层 `equal_rank_average` 提供（design §11）。
- **实现入口**：`research.composites.implementations.linear_rank:rank_average`
...
> 数字自 `runs/platform/composites/linear_rank/summary.json` 的 `evaluation` 段快照
> （C2 证据 `governance/evidence/verification/R34/c2/compose_linear_rank.txt`）。

### 4.1 主指标

| 指标 | 值 |
|------|----|
| RankIC mean / t / IR | -0.043226 / -0.980713 / -0.185337 |
| 近 26 周 mean / t | -0.056441 / -1.258654 |
| PearsonIC mean / t | -0.053391 / -1.144793 |
| 分层 spread（D1−D10）/ 单调 | -0.001623 / true |
| 换手（monthly / quarterly） | 0.822222 / 0.933333 |
...
|------|----|-------------------------------|
| `equal_raw_average` | -0.070153 | 0.026927 |
| `equal_rank_average` | -0.049537 | 0.006311 |

### 4.3 判定

- RankIC 不显著（\|t\|≈0.98），相对最优成员与两类基线只有小幅正增量（0.006~0.027）；
  全表 rank（非逐日截面）口径与评估层逐日 baseline 不可直接互比（§2 口径注）。
- 示例性质：用于验证 polars rank 生态路径 + 索引/档案治理，不构成因子质量结论。

## 5. 稳定性

- **子期稳定**：`sign_consistent=0.357`、`direction_consistent_share=0.357`——方向稳定性弱。
- **换手/覆盖**：monthly 0.822 / quarterly 0.933；coverage 0.9655（交集 reject 口径）。
- **失效风险**：成员为 5 日短窗信号；全表 rank 口径混入时间维度，窗口变化可能改变排序。
...
governance/ops/heavy.sh platform/.venv/bin/factorlab compose research/composites/specs/linear_rank.yaml
# 2) 索引/配对门
platform/.venv/bin/python research/tools/factor_lib/build_composite_index.py --check
```

原始输出：`governance/evidence/verification/R34/c2/compose_linear_rank.txt`。

## 7. 迭代历史

| 日期 | 变体/版本 | 改动 | 增量 delta | 结论 |
|------|-----------|------|------------|------|
| 2026-09-19 | `linear_rank`（初始） | C2 代表法示例（polars rank 生态路径）真跑 | 0.006391 | 不显著；示例/治理用途 |
...
- **口径**：intersection + reject；全表 rank 与逐日截面 baseline 的差异见 §2。
