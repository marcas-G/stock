---
name: ridge
spec: research/composites/specs/ridge.yaml
window: "2026-09-01 ~ 2026-09-16"
status: draft
snapshot: 未真跑（平台 venv 缺 scipy，pending #18 锁环境欠账；装库后按 §6 转真跑）
---

# ridge 合成分档案（Plan CX-C2 代表法示例）
...
| 版本项 | 值 |
|--------|----|
| definition_hash / cache_key / output_hash | **未产出**（缺 scipy，compose fail fast，无 runs 产物） |
| implementation.source_hash | `ridge_pls_pca.py` 当前内容（重跑时以 provenance 为准） |
| params_hash | 未产出 |

## 2. 方法与参数

- **方法**：Ridge 闭式解——`w = (XsᵀXs + αI)⁻¹ Xsᵀz`，`y = Xs w`（`scipy.linalg.solve`）；
  预处理 = 列 z-score（ddof=0），内部目标 `z` = z-score 逐行均值（stateless `X→y` 无外部
  y，design §12 边界内自定目标，design §6 框架不解析）。
- **实现入口**：`research.composites.implementations.ridge_pls_pca:ridge`
...
**未评估**：平台 venv 无 scipy（`lib_probe.txt: scipy absent`），`factorlab compose` 在
compute 入口 fail fast（exit 1，不落半成品）：

```
ImportError: scipy 未安装：ridge 示例需要 scipy（平台 venv 不新增依赖；装入 scipy 后本示例即可运行）
```

原始输出：`governance/evidence/verification/R34/c2/compose_ridge.txt`。
...
# 1) 装入 scipy（平台 venv 不新增依赖是 C2 期约束；装库后本示例即可真跑）
platform/.venv/bin/python -m pip install scipy
# 2) 合成分
governance/ops/heavy.sh platform/.venv/bin/factorlab compose research/composites/specs/ridge.yaml
# 3) 索引/配对门
...
|------|-----------|------|------------|------|
| 2026-09-19 | `ridge`（初始） | C2 示例落库（scipy 惰性 import，缺库 fail fast） | — | 缺库未真跑；装库后可转真跑 |

## 8. 风险与备注

- **未真跑**：本档案无评估数字；不得被引用为质量结论。C2 测试 `importorskip` 自动在装库后转真跑。
- **成员为证据 fixtures**：`cx_demo_x1/x2` 不在 `research/factor/` 因子库索引内、无因子档案。
- **自定目标**：`z` = 标准化后逐行均值（无外部 y）；口径由实现自定并记录 provenance（design §6）。
