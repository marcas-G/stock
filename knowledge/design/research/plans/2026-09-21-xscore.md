...
**Architecture:** numpy 优先（平台 venv 无 sklearn/scipy）；模块 `research/tools/xscore/` 只做 fit(X,y)/predict(X)→score；复用 `lab/autoencoder42/` 的面板、walk-forward、指标与组合消费脚本做验收。

**Spec:** `knowledge/design/research/specs/2026-09-21-xscore-design.md`
...
- 证据：`governance/evidence/verification/R37/xscore/`；结果 `quantresearch/results/2026-09-xscore/`。

---

### Task 1：模块骨架 + N（标准化）+ C（校准）

**Files:** Create `quantresearch/lab/xscore/{__init__,normalize,calibrate}.py`、`tests/test_normalize.py`、`tests/test_calibrate.py`
...
- [ ] 实现 `calibrate(s)`。
- [ ] 提交 `feat(xscore): N/C（robust z 标准化与校准）`
...
- [ ] 提交 `feat(xscore): 显式特征扩充（Core/Extended）+ 训练窗结构过滤`
...
- [ ] 提交 `feat(xscore): HuberRidge/ElasticNet/PLS 聚合器`
...
- [ ] 提交 `feat(xscore): pipeline 装配与泄漏硬门`

### Task 5：M0–M4 阶梯实验（42 因子面板，walk-forward）

**Files:** Create `run_ladder.py`；输出 `results/2026-09-xscore/`
...
- [ ] 证据 `R37/xscore/` + 提交 `docs(verification): xscore M0–M4 阶梯结果`
