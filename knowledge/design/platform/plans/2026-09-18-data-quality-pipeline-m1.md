...
- [x] **Step 1: 失败测试**：policy 缺字段 → `ValueError`（点名缺失项）；旧版本号加载 → 兼容/拒绝规则明确；规则目录含 §4 ①-⑦ 的 rule_id 常量。
- [x] **Step 2: 红→实现**（YAML 直读 + dataclass；阈值字段名与 spec §3.2 完全一致）。
- [x] **Step 3: 提交** `feat(dq): dq_policy 与规则模型（Plan DQ-M1 T1）`
...
- [x] **Step 1: 失败测试**（每规则：正例 + 反例；INFO 例外）：
...
- [x] **Step 2: 红→实现**（polars 向量化；输出稳定排序，便于分区聚合）。
- [x] **Step 3: 提交** `feat(dq): daily 行级校验器（Plan DQ-M1 T2）`
...
- [x] **Step 1: 失败测试**：完全相同行 dedup（保留策略：全列一致才删）；时间解析/时区规范化幂等；
  **PK payload 冲突 → 两行都进 quarantine**（断言 clean 中无该键）；quarantine 落盘结构 + 索引字段（rule_id/reason/count）。
- [x] **Step 2: 红→实现**。
- [x] **Step 3: 提交** `feat(dq): 确定性修复与行级隔离（Plan DQ-M1 T3）`
...
- [x] **Step 1: 失败测试**（边界值）：`error_rate` 恰等于 `pass.max_error_rate` → PASS（≤ 语义）；
...
- [x] **Step 2: 红→实现**（判定顺序与 spec §3.1 逐字一致）。
- [x] **Step 3: 提交** `feat(dq): 聚合/系统性检测/PRE-INGEST 门（Plan DQ-M1 T4）`
...
- [x] **Step 1: 失败测试**：伪 raw parquet → clean staging 落盘（`data/staging/ashare_daily/<partition>/`）；
  PRE-INGEST FAIL 时：**不产生 staging 完成标记、不触发 ingest**（spy 断言 ingest 未被调用）。
- [x] **Step 2: 红→实现**（CLI/阶段链最小接线，`--dry-run` 不落盘）。
- [x] **Step 3: 提交** `feat(dq): clean staging 与 pan_update 接线（Plan DQ-M1 T5）`
...
- [x] **Step 2: 红→实现**（发布原子写；`raw_lineage.sha256` 取自 raw 文件）。
- [x] **Step 3: 提交** `feat(dq): post-ingest audit 与 health 发布（Plan DQ-M1 T6）`
...
- [x] **Step 1: 失败测试**（拒绝矩阵）：PASS→过；DEGRADED 默认拒、opt-in 后过且 manifest 写入（断言五个字段）；
...
- [x] **Step 2: 红→实现**（只读 health JSON；异常文案含 dataset/partition/status/指引）。
- [x] **Step 3: 提交** `feat(read): require_dataset 读取门与产物记录（Plan DQ-M1 T7）`
...
- [x] **Step 1: 失败测试**（小样本）：扫描给定分区范围 → 汇总问题清单（按日期/字段/规则计数）+ 影响分析骨架；
  **禁止行为断言**：审计过程不得写 canonical / 不得改 parquet（只写 health/verification 标记与报告）。
- [x] **Step 2: 红→实现**；对存量分区写 `verification_state=LEGACY_UNVERIFIED, health_status=UNKNOWN`。
- [x] **Step 3: 真跑 daily 全史**：出问题清单 + 影响分析（顶层：高影响问题清单，如 circ_mv/退市 adj/跨频）。
- [x] **Step 4: 提交** `feat(dq): daily 全史体检与存量打标（Plan DQ-M1 T8）`

### Task 9：M1 端到端验收

- [x] 真实 `make data-update`（daily 类别）：staging/quarantine/health 全产出；`verify` 绿；
- [x] 反例注入：构造 FATAL 分区 → PRE-INGEST FAIL、canonical 未写入、stage 标记未落；
  构造 DEGRADED 分区 → 默认读取被拒、opt-in 通过且 manifest 完整；
- [x] 读取拒绝矩阵表（PASS/DEGRADED/FAIL/UNKNOWN × freshness × completeness）；
- [x] 证据 `governance/evidence/verification/R32/`；台账（reviews）登记执行状态。
