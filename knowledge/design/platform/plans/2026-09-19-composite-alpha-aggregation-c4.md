...
- [x] `signal: composites/<name>` 自动识别（+显式 `signal_kind` 覆盖）；`StrategyDoc.signal_kind`（default factor）；
- [x] 加载分派：factor→`load_signal_artifact(results_dir/name)`；composite→`runs/platform/composites/<name>/`（复用 `app/composite/artifact.read_composite_artifact`），包 `SignalArtifact(frame, meta)`；
- [x] 失败清晰：kind 不符/产物缺失/非 composite 目录 → 点名报错；
- [x] 测试：composite 引用成功（合成产物）；factor 路径零回归；prefix/kind 冲突处理。
- [x] 提交 `feat(strategy): 策略引用 composite 信号（Plan CX-C4 T1）`
...
- [x] `WeightingSpec.method: Literal["equal_weight","score_weighted"]`；
- [x] constructor：`s=signal×direction`，Top-K 后 `s'=max(s,0)`，`w=gross×s'/Σs'`；**Σs'==0→all-cash**；
- [x] 测试（手算）：归一逐值；全负/全零→all-cash（0 rows 但 decision_date 在）；与 equal_weight 的差异用例；方向翻转对称；
- [x] 提交 `feat(strategy): score_weighted 加权（Plan CX-C4 T2）`

### Task 3：E2E（T3，串联）
- [x] 示例策略 `$QUANTRESEARCH_ROOT/strategy/cx_demo_score_weighted.yaml`（signal: composites/cx_demo；portfolio: score_weighted）；
- [x] 真跑（CH 小窗）：run_strategy → NAV/持仓产物；`equal_weight` vs `score_weighted` 对照表；
- [x] 证据 `governance/evidence/verification/R34/c4/`；提交 `docs(verification): R34 C4 端到端验收`

### Task 4：验收
- [x] factor 策略零回归（既有 e2e）；composite 引用全链；score_weighted 手算一致；all-cash 边界；门相关项绿。
