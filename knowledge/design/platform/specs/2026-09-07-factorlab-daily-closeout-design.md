...
> **R04 勘误（2026-09-16）**：上述对应计划未随本仓存档（外部会话计划名）；
> 实施记录见本文 §13 验证记录 / §14 提交序列与 `docs/verification/` 证据。正文不改。
...
**R03-I8 追加（2026-09-16）**：分段工作流落定于 `interface.md` §6
`run_backtest` 条目后——显式 `decision_range` 分段、段间显式重基并**丢弃
边界 return**；不提供自动分段 helper / 不自动重基 NAV（分段 run 独立初始化
`initial_cash` + 空仓位——段间持仓/资金连续性丢失，M8-06A §3.1；B14 测试锁
