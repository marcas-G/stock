...
**hard 类台账（截至 2026-09-11 09:55，6 例 / 5 个月）**：`301308.SZ@20250922`(0.89963)
/ `301308.SZ@20251030` / `300418.SZ@20260113`(0.89616) / `301308.SZ@20260807`(0.89981)
/ `301308.SZ@20260507`(**0.89367**) / `301308.SZ@20260511`(0.89799) —— 全部
`m1a_presence`、全部紧贴 0.90 日级地板（0.8937–0.8998 窄带）、**非单一标的但仍高度
集中**（5/6 是 `301308.SZ`，其 2.60% 的天数贴门见 §3b 分布表）→ 支持 §3b 归类为
"极高消息速率标的的 δ 滞后尾部（方法边界）"；累积率 6/57,077 ≈ 0.011% ≪ 0.1%。
...
**分布级佐证（2026-09-11，57,077 个已归档 code-day，13 runs）**：日级 presence 总体
中位 = **0.99440**（p1 = 0.94522），hard 类标的是**贴门分布**：

| 标的 | code-day | min | p1 | p5 | 中位 | <0.90 天数 |
|---|---|---|---|---|---|---|
| `301308.SZ` | 192 | 0.89367 | 0.89799 | 0.90423 | **0.92631** | **5（2.60%）** |
| `300418.SZ` | 192 | 0.89616 | 0.90187 | 0.93156 | 0.97002 | 1（0.52%） |
| （全体） | 57,077 | — | 0.94522 | 0.96401 | 0.99440 | 6（**0.0105%**） |

→ 这是**关于标的、而非引擎的陈述**：`301308.SZ` 的中位 presence 比总体低 6.8 个
百分点，其 5 个硬日全部落在 0.8937–0.8998 窄带（紧贴地板，无一崩塌）——与 δ 滞后机制
自洽（消息速率越高 → 固定 500ms 吸收窗的窗外比例越大）。**非随机失败、未见第二类
根因**；对交付物的实际影响 = 0.0105% 的 code-day，比 0.1% 阈值低一个数量级。
