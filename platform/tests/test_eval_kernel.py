"""评估内核契约测试：键集/统计/边界/错误路径/与平台 ic_series 逐期对拍。

R30 Task 15（D12）：内核已并入 `factorlab.core.eval.kernel`（单一实现）——
本文件即其行为契约锚点；P-6 端口文档仍是 `ports/eval_kernel.py`。
契约文档：knowledge/design/platform/specs/2026-08-26-quant-core-contract.md
（R18 取回入树；本文件同批取回，import 路径按 R30 Task 15 的合并后层名修正）。
"""
import datetime
import pathlib
import random
...
def test_contract_keys_full_structure():
    ...
    r = kernel.evaluate_factor(*_args(_panel()), "_factor", 1)
...
def test_direction_flips_spread_and_zero_means_negative():
    ...
    up = kernel.evaluate_factor(*_args(_panel()), "_factor", 1)
    down = kernel.evaluate_factor(*_args(_panel()), "_factor", -1)
    zero = kernel.evaluate_factor(*_args(_panel()), "_factor", 0)
...
def test_none_rejected_with_typeerror():
    # 错误路径：None → TypeError("must be real number")（内核实测契约）
    dates, codes, signals, fwd = _args(_panel())
    with pytest.raises(TypeError, match="must be real number"):
        kernel.evaluate_factor(dates, codes, signals[:-1] + [None], fwd, "_factor", 1)
    with pytest.raises(TypeError, match="must be real number"):
        kernel.evaluate_factor(dates, codes, signals, fwd[:-1] + [None], "_factor", 1)


def test_mismatched_lengths_rejected():
    dates, codes, signals, fwd = _args(_panel())
    with pytest.raises(ValueError, match="长度不一致"):
        kernel.evaluate_factor(dates, codes, signals[:-1], fwd, "_factor", 1)


def test_empty_panel_nan_structure():
    # 边界：空面板 → 全 nan 结构（实测不崩溃），coverage 归零
    r = kernel.evaluate_factor([], [], [], [], "_factor", 1)
...
def test_nan_rows_tolerated_and_excluded():
    # 边界：NaN 容忍（实测不崩溃）；内核假设 NaN 视为无效观测 → 计入 coverage 差额
    dates, codes, signals, fwd = _args(_panel())
    signals[3] = float("nan")
    r = kernel.evaluate_factor(dates, codes, signals, fwd, "_factor", 1)
    assert r["coverage"]["total_rows"] == 120
    assert r["coverage"]["valid_rows"] == 119
    assert r["coverage"]["pct_valid"] == pytest.approx(119 / 120, abs=1e-3)  # 内核保留 4 位
    assert r["n_weeks"] == 12  # 该周仍计数（119 有效观测）

    # 全 NaN → 全部周退化：n_weeks 保留、ic 统计 nan（不崩溃）
    r2 = kernel.evaluate_factor(dates, codes, [float("nan")] * 120, fwd, "_factor", 1)
    assert r2["n_weeks"] == 0  # 无有效观测周
    assert r2["ic"]["mean"] != r2["ic"]["mean"]


def test_two_stock_week_counted():
    # 边界：2 只股票的周仍计数（内核实测：平台 CLI 2 只股票断言 n_weeks>=1；
    # ic_series 的 MIN_STOCKS=3 是平台稳健性选择，内核对拍用 ≥3 只面板避开）
    df = pl.DataFrame({
    ...
    r = kernel.evaluate_factor(*_args(df), "_factor", 1)
    assert r["n_weeks"] == 1
...
def test_constant_fwd_weeks_counted_ic_nan():
    ...
    df = _panel()
    r = kernel.evaluate_factor(*_args(df.with_columns(pl.lit(0.01).alias("fwd"))),
                                   "_factor", 1)
    assert r["n_weeks"] == 12
    assert r["ic"]["mean"] != r["ic"]["mean"]  # 全部退化 → nan


def test_matches_ic_series_per_period():
    # 对拍：内核周 IC 序列统计 == 平台 ic_series（同源 pl.corr spearman）
    # 构造每周 ≥3 只（MIN_STOCKS 差异区外）；ic_series 的 None 周 = 内核过滤的退化周
    df = _panel(weeks=8, stocks=12, seed=42)
    r = kernel.evaluate_factor(*_args(df), "_factor", 1)
    wic = ic_series(df.rename({"fwd": "forward_return_5d"})).drop_nulls("ic")
    ...
    # 单周面板对拍：内核 ic.mean 即该周 spearman
    one = df.head(12)
    r1 = kernel.evaluate_factor(*_args(one), "_factor", 1)
    w1 = ic_series(one.rename({"fwd": "forward_return_5d"}))["ic"].drop_nulls()
    assert r1["n_weeks"] == 1
...
def test_degenerate_weeks_excluded_from_t_and_sign_denominators():
    ...
    r = kernel.evaluate_factor(*_args(pl.DataFrame(rows)), "_factor", 1)
...
def test_decile_ties_row_order_invariant_numeric():
    ...
    a = kernel.evaluate_factor(*_args(pl.DataFrame(base)), "_factor", 1)
    b = kernel.evaluate_factor(*_args(pl.DataFrame(list(reversed(base)))), "_factor", 1)
...
def test_turnover_exact_value_and_row_order_invariant():
    ...
    assert kernel._turnover(df, 4) == pytest.approx(0.25)   # A 变档 1/4
    shuffled = list(rows)
    random.Random(0).shuffle(shuffled)
    df_sh = pl.DataFrame(shuffled).with_columns(pl.col("_decile").cast(pl.Int64))
    assert kernel._turnover(df_sh, 4) == pytest.approx(0.25)
    ...
    assert kernel._turnover(df2, 4) == pytest.approx(0.5)
...
def test_turnover_nan_when_buckets_insufficient():
    ...
    df = pl.DataFrame(rows).with_columns(pl.col("_decile").cast(pl.Int64))
    assert kernel._turnover(df, 4) != kernel._turnover(df, 4)   # monthly 需 8 周
    assert kernel._turnover(df, 12) != kernel._turnover(df, 12)  # quarterly 更不足
