...
| 命令 | 行为 |
|---|---|
| `sync` | 逐类别差集下载；新文件/同名 size 变 → 下载并记账；同名同 size → 跳过 |
| `build` | 逐类别跑阶段链（含 CH 灌入），成功才落阶段标记 |
| `publish` | 与 `build` 同义（同一阶段标记；链已含发布面） |
| `verify` | `platform/tools/ch_ingest/reconcile.py` 全量对账，退出码原样传播 |
| `all` | `sync` → `build` → `publish`(跳过) → `verify`；任一失败即停并返回非零 |

- `--dry-run`：只打印每类别 `to_fetch` 清单与 `--prune` 计划；不取链、不下载、不写 state。
...
- **freshness 闩锁**：本次 `sync` 有新增/变更（`downloaded` 非空）→ 清该类别 `stages[cat]`
  全部标记，下次 `build` 重跑整链；无新增 → 阶段幂等跳过（命令不再执行）。
- 半成品防护：下载先写 `<rel_path>.part`，size 校验通过才 `os.replace`；失败清 `.part`、不记账。

## Cookie 维护

- 仓根 `quark_cookies.txt`（已 gitignore；`chmod 600`）；也可用 `QUARK_COOKIE_FILE` 覆盖。
- 失效（401/空 stoken）/缺失/空文件 → `sync`/`all` 启动即 exit 2，文案含 cookie 路径；
  **不静默重试打转**。更新方式：浏览器登录夸克 → 导出 Cookie 串 → 覆盖该文件。
...
（差集按本地记账跳过已下文件；人工件进入对应阶段链）。**manual 不影响退出码**。
...
- 白名单 env 透传子命令：`FACTORLAB_DATA_BACKEND`、`FACTORLAB_MAX_MEMORY`、
  `FACTORLAB_MIN_AVAILABLE_MEMORY`、`PYARROW_JEMALLOC`（其余继承父环境）。
