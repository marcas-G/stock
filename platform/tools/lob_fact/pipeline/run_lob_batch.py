...
# ---------- 表 → 事件映射：R11 已移入 core/events.py（core 是叶子）----------
# 此处 re-export：历史调用点 `R.orders_to_events(...)` / `R.parquet_events(...)`（含测试）不变。
from core.events import (cancels_to_events, ex_of as _ex_of,  # noqa: E402,F401
                         orders_to_events, parquet_events, trades_to_events)
