...
def ic_series(panel: pl.DataFrame, target: str = "forward_return_5d") -> pl.DataFrame:
    ...
    **周期无关**（D9/R30 Task 13 改名自 weekly_ic）：daily 面板 → 逐日 IC 曲线、
    ...
    return (
        ...
        .with_columns(
            pl.when(
                pl.col("n_valid").fill_null(0).lt(MIN_STOCKS)
                | pl.col("ic").is_null()
                | ~pl.col("ic").is_finite()
