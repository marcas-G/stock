...
- 契约 4 篇 → `../knowledge/contracts/`；平台设计/计划 → `../knowledge/design/platform/{specs,plans}/`；运行产物 → 研究产物区 `<QUANTRESEARCH_ROOT>/results/platform/`（`settings.results_dir` 默认；仓内 `runs/platform` 为兼容软链）。
