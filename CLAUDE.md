...
- 提交前跑对应测试：平台 `cd platform && .venv/bin/python -m pytest -q`（基线 **2486 passed / 13 skipped**）；
  研究 `emb/bin/python -m pytest research/tools -q`（基线 **183 passed / 1 skipped**）+ T1 `platform/.venv/bin/python -m pytest research/tools/strategies/tests -q`（**24**）。
